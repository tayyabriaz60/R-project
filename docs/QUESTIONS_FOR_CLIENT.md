...
**Please answer BLOCKING Study 1 items before coding is frozen.** Study 2–3-only items can wait if Study 1 must ship first, but several Study 1 answers will also lock Studies 2–3 (effect-size convention, Wilcoxon zeros, fallback process).
...
### Q5 — BLOCKING Study 1
- **Where:** Dictionary §10 Object Name filters vs all synthetic Study 1–3 files (`Object Name` is 100% missing). README: only `Response Type = response` rows were generated.
...
### Q6 — BLOCKING Study 1
- **Where:** Dictionary §3 `participant_id` ← Participant Public ID; Study 1 synthetic Public ID = `BLINDED`; README says use Participant Private ID.
