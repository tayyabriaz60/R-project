...
| 2026-09-24 | Start Study 2 with load/QC only | SAP §4; Fatimah “start study2” | Combined synthetic file split by Dict §10 Clone task names. Researcher drop uses synthetic Public ID `S2_RESEARCHER_EXCLUDE` only. Real researcher ID (Q18) and real task ids (Q33) stay NA and `stop()`. Study 1 Q7 lock is not inherited (Q32). `run_all.R` stays Study 1 only. Kaggle of this load: NOT EXECUTED. |
